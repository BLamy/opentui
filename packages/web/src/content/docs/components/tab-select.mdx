...
```ts example
import { TabSelectRenderable, TabSelectRenderableEvents, type TabSelectOption, createCliRenderer } from "@opentui/core"
...
const tabs = new TabSelectRenderable(renderer, {
  ...
  options: [
    ...
    { name: "Settings", description: "Update app preferences" },
  ],
})
...
const tabs = new TabSelectRenderable(renderer, {
  ...
  tabWidth: 20,
  options: [
    { name: "Overview", description: "Summary panel" },
    { name: "Logs", description: "Recent system output" },
...
const tabs = TabSelect({
  ...
  tabWidth: 20,
  options: [
    { name: "Overview", description: "Default landing tab" },
    { name: "Tasks", description: "Pending work items" },
    { name: "Activity", description: "Recent updates" },
  ],
})

const currentIndex = tabs.getSelectedIndex()
console.log("Currently selected tab:", currentIndex)
